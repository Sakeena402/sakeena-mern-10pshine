...
//   await request(`/notes/${noteId}/pin`, { method: "PATCH" })
//   alert("Note pin status updated!")
// }

// const handleLockNote = async (noteId: string, enable: boolean) => {
//   const pin = enable ? prompt("Enter 4-digit PIN") : undefined
//   await request(`/notes/${noteId}/pin`, {
//     method: "PATCH",
//     body: JSON.stringify({ pinCode: pin, enable }),
//   })
//   alert(enable ? "PIN added!" : "PIN removed!")
// }
...
export default function NotesPage() {
  ...
  const { data: notes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (await request(url)).notes
    },
  })

  // ✅ Mutations (auto refresh notes)
          ...
          <div className="max-w-6xl mx-auto">
            {isLoading ? (
              ...
              <div className="animate-fade-in">
                ...
                {activeSection === "all" && (
                  <section className="mb-12">
                    <h3 className="text-gray-800 font-serif font-semibold mb-3">
                      Helpful Tips
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
                      {HELP_NOTES.map((helpNote, index) => (
                ...
                <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredNotes.map((note: INote) => (
                    <NoteCard
                      ...
                      onLock={(id, enable, pinCode) => lockNote.mutate({ id, enable, pinCode })}
